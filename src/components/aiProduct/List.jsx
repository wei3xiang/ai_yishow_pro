  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiProduct) =>
       ...
       `${aiProduct.aiProductCategory}`.includes(searchTerm) ||
       `${aiProduct.aiProductInventory}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiProduct) => (
            <tr key={aiProduct.uuid}>
              ...
              <td>{aiProduct.aiProductCategory}</td>
              <td>{aiProduct.aiProductInventory}</td>
              <td>
                <button

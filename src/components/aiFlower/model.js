...
export default {
  ...
  effects: {
    * list({ payload }, { call, put, select }) {
      ...
      const service = AiFlowerControllerApi.of(API);
      const { data: { list, pageNum: __pageNum, total } } = yield call(() => service.fetchAiFlowerList(opts));
      yield put({
        type: 'updateList',
        payload: {
          list,
    ...
    * create({ payload }, { call, put, select }) {
      ...
      yield call(() => service.saveAiFlower(payload));
    ...
    * update({ payload }, { call, put, select }) {
      const service = AiFlowerControllerApi.of(API);
      yield call(() => service.updateAiFlower(payload));
    ...
    * remove({ payload: { uuid } }, { call, put, select }) {
      ...
      yield call(() => service.removeAiFlower(uuid));
    ...
    * toUpdate({ payload: { uuid } }, { call, put, select }) {
      const service = AiFlowerControllerApi.of(API);
      const { data: updateModel } = yield call(() => service.fetchAiFlowerDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { updateModel },
      });
    ...
    * toRemove({ payload: { uuid } }, { call, put, select }) {
      const service = AiFlowerControllerApi.of(API);
      const { data: removeModel } = yield call(() => service.fetchAiFlowerDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { removeModel },
      });

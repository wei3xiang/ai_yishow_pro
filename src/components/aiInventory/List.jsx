  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiInventory) =>
       ...
       `${aiInventory.ai_good_uuid}`.includes(searchTerm) ||
       `${aiInventory.ai_inventory_number}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiInventory) => (
            <tr key={aiInventory.uuid}>
              <td>{aiInventory.ai_good_uuid}</td>
              <td>{aiInventory.ai_inventory_number}</td>
              <td>
                <button

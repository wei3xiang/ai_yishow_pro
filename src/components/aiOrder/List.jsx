  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiOrder) =>
       ...
       `${aiOrder.aiUserId}`.includes(searchTerm) ||
       `${aiOrder.aiProductId}`.includes(searchTerm) ||
       `${aiOrder.aiOrderAmount}`.includes(searchTerm) ||
      ...
      >
        ...
        <tbody>
          {filteredList.map((aiOrder) => (
            <tr key={aiOrder.uuid}>
              <td>{aiOrder.aiUserId}</td>
              <td>{aiOrder.aiProductId}</td>
              <td>{aiOrder.aiOrderAmount}</td>

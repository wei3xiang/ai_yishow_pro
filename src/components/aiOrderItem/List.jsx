  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiOrderItem) =>
       ...
       `${aiOrderItem.ai_inventory_uuid}`.includes(searchTerm) ||
       `${aiOrderItem.ai_quantity}`.includes(searchTerm) ||
       `${aiOrderItem.ai_sub_total}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiOrderItem) => (
            <tr key={aiOrderItem.uuid}>
              ...
              <td>{aiOrderItem.ai_inventory_uuid}</td>
              <td>{aiOrderItem.ai_quantity}</td>
              <td>{aiOrderItem.ai_sub_total}</td>
              <td>
                <button

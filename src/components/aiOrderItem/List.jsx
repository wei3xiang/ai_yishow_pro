  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiOrderItem) =>
       ...
       `${aiOrderItem.ai_flower_uuid}`.includes(searchTerm) ||
       `${aiOrderItem.ai_quantity}`.includes(searchTerm) ||
       `${aiOrderItem.ai_item_price}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiOrderItem) => (
            <tr key={aiOrderItem.uuid}>
              ...
              <td>{aiOrderItem.ai_flower_uuid}</td>
              <td>{aiOrderItem.ai_quantity}</td>
              <td>{aiOrderItem.ai_item_price}</td>
              <td>
                <button

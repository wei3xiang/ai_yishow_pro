...
const Index = () => {
        ...
        layero.find("#modalOk").on("click", () => {
          const updatedItem = {
            ...aiCustomer,
              ...
              aiCustomerEmail: layero.find('input[name="aiCustomerEmail"]').val(), 
              aiCustomerPhone: layero.find('input[name="aiCustomerPhone"]').val(), 
              aiCustomerAddress: layero.find('input[name="aiCustomerAddress"]').val(), 
          };
          setList(
  ...
  const handleCreate = () => {
    const newItem = { uuid: "",  aiCustomerName: "",  aiCustomerEmail: "",  aiCustomerPhone: "",  aiCustomerAddress: "", };
    ...
    layui.layer.open({
      ...
      success: function (layero, index) {
        ...
        layero.find("#modalOk").on("click", () => {
          ...
          const aiCustomerEmail = layero.find('input[name="aiCustomerEmail"]').val();
          const aiCustomerPhone = layero.find('input[name="aiCustomerPhone"]').val();
          const aiCustomerAddress = layero.find('input[name="aiCustomerAddress"]').val();
          setList([...list, { ...newItem, uuid: Date.now(), aiCustomerName,aiCustomerEmail,aiCustomerPhone,aiCustomerAddress, }]);
          layui.layer.close(index);
        });
      },
    });

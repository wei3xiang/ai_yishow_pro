...
const Index = () => {
  ...
  const handleCreate = () => {
    const newItem = { uuid: "",  aiCustomerName: "",  aiEmail: "",  aiPhone: "",  aiAddress: "", };
      ...
      success: function (layero, index) {
        ...
        layero.find("#modalOk").on("click", () => {
          ...
          const aiAddress = layero.find('input[name="aiAddress"]').val();
           if (aiCustomerName && aiEmail && aiPhone && aiAddress
           ) {
            setList([...list, { ...newItem, uuid: Date.now(), aiCustomerName,
            aiEmail,
            aiPhone,
            aiAddress,
             }]);

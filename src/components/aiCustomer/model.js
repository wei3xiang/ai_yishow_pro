...
import {AiCustomerControllerApi} from 'dubhe-sdk-review';
...
export default {
  ...
  effects: {
    * list({ payload }, { call, put, select }) {
      ...
      const service = AiCustomerControllerApi.of(API);
      const { data: { list, pageNum: __pageNum, total } } = yield call(() => service.fetchAiCustomerList(opts));
      yield put({
        type: 'updateList',
        payload: {
          list,
    ...
    * create({ payload }, { call, put, select }) {
      ...
      yield call(() => service.saveAiCustomer(payload));
    ...
    * update({ payload }, { call, put, select }) {
      const service = AiCustomerControllerApi.of(API);
      yield call(() => service.updateAiCustomer(payload));
    ...
    * remove({ payload: { uuid } }, { call, put, select }) {
      ...
      yield call(() => service.removeAiCustomer(uuid));
    ...
    * toUpdate({ payload: { uuid } }, { call, put, select }) {
      const service = AiCustomerControllerApi.of(API);
      const { data: updateModel } = yield call(() => service.fetchAiCustomerDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { updateModel },
      });
    ...
    * toRemove({ payload: { uuid } }, { call, put, select }) {
      const service = AiCustomerControllerApi.of(API);
      const { data: removeModel } = yield call(() => service.fetchAiCustomerDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { removeModel },
      });

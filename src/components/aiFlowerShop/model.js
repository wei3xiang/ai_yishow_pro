...
import {AiFlowerShopControllerApi} from 'dubhe-sdk-review';
...
export default {
  ...
  effects: {
    * list({ payload }, { call, put, select }) {
      ...
      const service = AiFlowerShopControllerApi.of(API);
      const { data: { list, pageNum: __pageNum, total } } = yield call(() => service.fetchAiFlowerShopList(opts));
      yield put({
        type: 'updateList',
        payload: {
          list,
    ...
    * create({ payload }, { call, put, select }) {
      ...
      yield call(() => service.saveAiFlowerShop(payload));
    ...
    * update({ payload }, { call, put, select }) {
      const service = AiFlowerShopControllerApi.of(API);
      yield call(() => service.updateAiFlowerShop(payload));
    ...
    * remove({ payload: { uuid } }, { call, put, select }) {
      ...
      yield call(() => service.removeAiFlowerShop(uuid));
    ...
    * toUpdate({ payload: { uuid } }, { call, put, select }) {
      const service = AiFlowerShopControllerApi.of(API);
      const { data: updateModel } = yield call(() => service.fetchAiFlowerShopDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { updateModel },
      });
    ...
    * toRemove({ payload: { uuid } }, { call, put, select }) {
      const service = AiFlowerShopControllerApi.of(API);
      const { data: removeModel } = yield call(() => service.fetchAiFlowerShopDetail(uuid, {}));
      yield put({
        type: 'refreshModel',
        payload: { removeModel },
      });

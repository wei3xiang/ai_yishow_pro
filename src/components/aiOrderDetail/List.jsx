  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiOrderDetail) =>
       ...
       `${aiOrderDetail.ai_flower_uuid}`.includes(searchTerm) ||
       `${aiOrderDetail.ai_flower_count}`.includes(searchTerm) ||
       `${aiOrderDetail.ai_flower_total_price}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiOrderDetail) => (
            <tr key={aiOrderDetail.uuid}>
              ...
              <td>{aiOrderDetail.ai_flower_uuid}</td>
              <td>{aiOrderDetail.ai_flower_count}</td>
              <td>{aiOrderDetail.ai_flower_total_price}</td>
              <td>
                <button

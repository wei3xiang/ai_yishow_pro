  ...
  const filteredList = useMemo(() => {
    return data.filter(
      (aiEmployee) =>
       ...
       `${aiEmployee.ai_employee_gender}`.includes(searchTerm) ||
       `${aiEmployee.ai_employee_age}`.includes(searchTerm) ||
       `${aiEmployee.ai_employee_position}`.includes(searchTerm)
    );
  }, [data, searchTerm]);
        ...
        <tbody>
          {filteredList.map((aiEmployee) => (
            <tr key={aiEmployee.uuid}>
              ...
              <td>{aiEmployee.ai_employee_gender}</td>
              <td>{aiEmployee.ai_employee_age}</td>
              <td>{aiEmployee.ai_employee_position}</td>
              <td>
                <button
